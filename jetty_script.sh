...
java \
    ...
    -jar /opt/jetty/start.jar --add-to-startd=setuid,jmx
cd /
...
echo "JETTY_HOME=/opt/jetty" > /etc/default/jetty
{
  ...
  echo "JAVA_OPTIONS='-Djetty.http.port=80 -Dcom.sun.management.jmxremote -Dcom.sun.management.jmxremote.ssl=false -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.port=1099'"
